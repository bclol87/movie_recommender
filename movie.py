...
st.markdown("""
    ...
    .section-title { font-size: 1.3rem; font-weight: 700; color: #e5e5e5; margin-bottom: 15px; margin-top: 20px;}

    /* Horizontal Scrolling Containers */
    .scroll-container { display: flex; flex-wrap: nowrap; overflow-x: auto; gap: 12px; padding-bottom: 40px; scroll-behavior: smooth; }
    ...
    .top10-wrapper { flex: 0 0 280px; display: flex; position: relative; cursor: pointer; margin-right: 40px; transition: transform 0.3s ease;}
    .top10-wrapper:hover { transform: scale(1.05); z-index: 20; }
    .top10-number { font-size: 14rem; font-weight: 900; color: #000; -webkit-text-stroke: 4px #555; position: absolute; left: -30px; bottom: -60px; z-index: 1; letter-spacing: -10px; line-height: 1;}
    .top10-card { flex: 1; aspect-ratio: 2/3; background-color: #222; border-radius: 4px; overflow: hidden; position: relative; z-index: 2; margin-left: 50px; border: 1px solid rgba(255,255,255,0.1); box-shadow: 5px 0 15px rgba(0,0,0,0.6);}
