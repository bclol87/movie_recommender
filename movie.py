...
st.markdown('<div class="sub-title">Smart AI Movie Recommendations</div>', unsafe_allow_html=True)
...
for i, (movie, row) in enumerate(recs.iterrows()):
    poster, overview, movie_link = fetch_movie_details(movie)
    ...
    with cols[i % 3]:
        ...
        st.markdown(
            f'<a href="{movie_link}" target="_blank" class="view-btn">View Details</a>',
            unsafe_allow_html=True
        )
